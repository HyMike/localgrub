  ...
  default: {
    auth: () => ({
     verifyIdToken: vi.fn().mockResolvedValue(
      { uid: 'u1', email: 'e@x.com' }) }),
     }}))

vi.mock('../utils/firebaseAdmin', () => ({
db: {
    collection: () => ({ doc: () => ({ get: vi.fn().mockResolvedValue({ exists: true, data: () => ({}) }) }) }),
...
const makeReq = (headers: Record<string, string> = {}, body: any = {}): Request =>
...
describe('createOrder', () => {
  it('returns 200 and orderId on success', async () => {
    ...
    const req = makeReq({ authorization: 'Bearer token' }, { itemName: 'Burger', quantity: 1 });
    const res = mockRes();
  ...
  it('returns 401 when unauthorized', async () => {
    vi.spyOn(auth, 'verifyUserToken').mockRejectedValue(new Error('Unauthorized'));
  ...
  it('returns 500 on unexpected error from service', async () => {
    ...
    vi.spyOn(orderSvc, 'createOrderInDatabase').mockRejectedValue(new Error('db fail'));
